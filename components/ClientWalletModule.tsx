...
interface ClientWalletProps {
    ...
    onUpdateClientBalance: (clientId: number, amountChange: number, reason: string, paymentMethod?: PaymentMethodType, accountId?: string) => void;
...
const ClientWalletModule: React.FC<ClientWalletProps> = ({ 
    ...
    const filteredClients = clients.filter(c => c.name.toLowerCase().includes(searchTerm.toLowerCase()) || c.dni.includes(searchTerm) || c.id.toString().includes(searchTerm));
    ...
    return (
        <div className="flex h-full gap-6 animate-in fade-in duration-500">
            ...
            <div className="w-1/3 flex flex-col gap-4">
                ...
                <div className="flex-1 overflow-y-auto space-y-2 custom-scrollbar pr-1">{filteredClients.map(client => (<div key={client.id} onClick={() => { setSelectedClient(client); setActiveTab('ACTIONS'); }} className={`p-4 rounded-xl border cursor-pointer transition-all ${selectedClient?.id === client.id ? 'bg-blue-50 border-blue-200 dark:bg-blue-900/20 dark:border-blue-800' : 'bg-white border-slate-100 hover:bg-slate-50 dark:bg-slate-800 dark:border-slate-700 dark:hover:bg-slate-700'}`}><div className="flex justify-between items-start"><div><div className="font-bold text-slate-700 dark:text-white text-sm">{client.name}</div><div className="text-xs text-slate-400">#{client.id} • DNI: {client.dni}</div></div><div className="text-right"><div className={`font-bold ${client.digitalBalance > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400'}`}>S/ {client.digitalBalance.toFixed(2)}</div><div className="text-[10px] text-slate-400 uppercase font-bold">Saldo</div></div></div></div>))}</div>
            </div>
                        ...
                        <div className="flex justify-between items-start mb-6 border-b border-slate-100 dark:border-slate-700 pb-6 shrink-0">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-slate-500 dark:text-slate-400 font-mono text-xs">#{selectedClient.id} • DNI: {selectedClient.dni}</p>
                                </div>
                            </div>
